...
Prompt Doctor 不只优化当前这一条提示词。

在你明确同意后，它会从反复出现、经过反馈验证的缺口中，逐步形成一份**只保存在本地的个人提示词画像**：你经常遗漏什么、偏好怎样的表达，以及哪种改法对你最有效。

```text
一次优化 → 真实反馈 → 匿名提取特征 → 重复验证 → 形成个人画像 → 下次优先复查
```

例如，如果你多次忘记定义受众和验收标准，Prompt Doctor 下次会优先检查这两项，而不是每次从零开始。
...
**通用案例库**负责让技能覆盖更多任务模式；**个人提示词画像**负责让它越来越懂你。两者分别授权：同意匿名收录案例，不等于同意建立个人画像。
...
└── references/
    ...
    ├── rewrites.md           # 8 类重写动作
    ├── examples.md           # 前后对照示例
    └── profile-schema.md     # 本地个人画像格式
```

首次同意建立画像后，本机会生成 `references/profile.local.md`；该文件已被 Git 忽略，不会随仓库提交。
