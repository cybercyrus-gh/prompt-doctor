...
Prompt Doctor 不会预先编造一堆模板，而是从真实使用中积累有代表性的案例。

当一次优化代表了新模式、效果得到验证，或暴露了新的提示词陷阱时，它会先询问你是否记录。只有明确同意后，才会匿名化追加到 `references/examples.md`。

- 普通反馈或重复模式不会询问
- 姓名、联系方式、财务和内部信息会被删除
- 案例按 **原始 → 诊断 → 变体** 保存
- 超过 10 条时先合并同类、删除被更好案例覆盖的旧条目

目标不是积累数量，而是用少量真实案例覆盖更多提示词模式。
